...
public class Nombre implements ValueObject<Nombre.Properties> {
    ...
    public Nombre(String nombres, String apellidos ) {
        ...
        if(mayorOIgual.mayorOIgualQue(this.apellidos.length(),5))throw new IllegalArgumentException("El apellido debe de tener minimo 5 caracteres");
        if(this.nombres.isBlank()) throw new IllegalArgumentException("El nombre no puede estar vacio");
        if(mayorOIgual.mayorOIgualQue(this.nombres.length(), 3))throw new IllegalArgumentException("El nombre debe de tener minimo 3 caracteres");
    }
