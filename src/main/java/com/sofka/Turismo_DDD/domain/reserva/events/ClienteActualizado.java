...
public class ClienteActualizado extends DomainEvent {
    ...
    public ClienteActualizado(ClienteID clienteID, Contacto contacto) {
        ...
        if (CompararVacioONulo.compararSiNuloOEsVacio(contacto.value().email()))throw new IllegalArgumentException("Para actualizar el cliente el email no puede ser nulo o vacio");
        if (CorreoElectronicoValido.emailValido(contacto.value().email())) throw new IllegalArgumentException("Para actualizar el cliente el email debe ser un correo electronico valido");
        if (CompararVacioONulo.compararSiNuloOEsVacio(contacto.value().telefono())) throw new IllegalArgumentException("Para actualizar el cliente el telefono no puede ser nulo o vacio");
        if (mayorOIgualQue(contacto.value().telefono().length(),10)) throw new IllegalArgumentException("Para actualizar el cliente el telefono debe ser mayor a 10 digitos");
    }
